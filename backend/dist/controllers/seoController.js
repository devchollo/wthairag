...
const seoRequestSchema = zod_1.z.object({
    url: zod_1.z.string().trim().url().max(2048),
    keywords: zod_1.z.string().trim().max(500).optional(),
    competitors: zod_1.z.array(zod_1.z.string().trim().url().max(2048)).max(MAX_COMPETITORS).optional(),
    crawlLimit: zod_1.z.number().int().min(1).max(MAX_CRAWL_PAGES).optional()
});
...
const segmentToken = (token, vocab) => {
    if (token.length < 12)
        return token;
    const lower = token.toLowerCase();
    const dp = Array(lower.length + 1).fill(null);
    dp[0] = { words: [], score: 0 };
...
const parseRobotsTxt = (robotsBody) => {
...
const getPageSpeedMetrics = async (targetUrl) => {
...
const getSerpInsights = async (targetUrl, keywords) => {
    ...
    for (const keyword of keywordSubset) {
        try {
            ...
            if (response.data?.answer_box)
                serpFeatures.add('Answer Box');
            if (response.data?.knowledge_graph)
                serpFeatures.add('Knowledge Panel');
            if (response.data?.people_also_ask?.length)
                serpFeatures.add('People Also Ask');
            if (response.data?.related_questions?.length)
                serpFeatures.add('Related Questions');
...
const analyzeKeywords = (text, keywords, meta) => {
...
const seoChecker = async (req, res) => {
    try {
        ...
        if (!parsed.success) {
            return (0, response_1.sendError)(res, 'Invalid input. Please provide a valid URL and optional keywords.', 400);
        }
        ...
        const response = await (0, safeFetch_1.safeFetch)(url, {
        ...
        let robotsInfo = { status: 'unknown', disallow: [], sitemaps: [] };
        try {
            const robotsResponse = await (0, safeFetch_1.safeFetch)(robotsUrl, {
                headers: { 'User-Agent': 'WorkToolsHub-SEO-Bot/1.0' }
            });
        ...
        let sitemapInfo = { status: 'unknown', url: sitemapUrl, urlCount: 0, errors: [] };
        const sitemapCandidates = robotsInfo.sitemaps.length > 0 ? robotsInfo.sitemaps : [sitemapUrl];
        for (const candidate of sitemapCandidates) {
            try {
                const sitemapResponse = await (0, safeFetch_1.safeFetch)(candidate, {
                    headers: { 'User-Agent': 'WorkToolsHub-SEO-Bot/1.0' }
                });
        ...
        while (queue.length > 0 && crawlResults.length < crawlCap) {
            ...
            try {
                const pageResponse = await (0, safeFetch_1.safeFetch)(current.url, {
                    headers: { 'User-Agent': 'WorkToolsHub-SEO-Bot/1.0' }
                });
                ...
                if (pageResponse.status >= 400)
                    issues.push(`HTTP ${pageResponse.status} response`);
                if (noindex)
                    issues.push('Meta robots set to noindex');
                if (disallowed)
        ...
        const keywordCoverage = keywordList.length
            ...
            : 0;
        const qualityScore = clamp(Math.round(40 + (keywordInsights.totalWords / 20) + (keywordCoverage * 40)), 0, 100);
        const competitorInsights = [];
        if (competitors && competitors.length > 0) {
            for (const competitor of competitors) {
                try {
                    const competitorResponse = await (0, safeFetch_1.safeFetch)(competitor, {
                        headers: { 'User-Agent': 'WorkToolsHub-SEO-Bot/1.0' }
                    });
